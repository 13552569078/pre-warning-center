  ...
  <hl-dialog
    ...
    <template v-if="uploadStatus === BEFORE_UPLOAD">
      <hl-group dir="vertical" full class="w-full" gap="var(--lg)">
        <hl-group dir="vertical" full class="w-full" gap="var(--md)">
          <h5>第一步：选择与视频设备关联的监测指标和接入对象</h5>
          <hl-form ref="formRef" :model="formData" :rules="rules" :label-position="['right', 'middle']" gap="var(--lg)" item-gap="var(--lg2)">
            <hl-form-item prop="targetInfoId" label="选择监测指标">
              <hl-select v-model="formData.targetInfoId" class="w-full" placeholder="请选择" filterable clearable>
                <hl-option v-for="item in targetInfoOptions" :key="item.code" :label="item.name" :value="item.code" />
              </hl-select>
            </hl-form-item>
            <hl-form-item prop="accessSourceObjectId" label="选择接入对象">
              <hl-select v-model="formData.accessSourceObjectId" class="w-full" placeholder="请选择" filterable clearable>
                <hl-option v-for="item in accessSourceOptions" :key="item.code" :label="item.name" :value="item.code" />
              </hl-select>
            </hl-form-item>
          </hl-form>
        </hl-group>
        ...
        <hl-group dir="vertical" class="w-full" :class="canUpload ? 'm-b-xl' : ''" gap="var(--sm)" full>
          ...
          <hl-upload
            ...
            :headers="headers"
            :action="`${API_DOMAIN}${API_PREFIX}${$props.action}?accessSourceObjectId=${formData.accessSourceObjectId}&targetInfoId=${formData.targetInfoId}`"
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <hl-button type="primary" class="m-l-md" :disabled="isUploading || !canUpload || !isFirstStepFinished" @click="handleUpload">确 定</hl-button>
      </span>
    </template>
  </hl-dialog>
</template>
...
import { IgetListAccessSourceObjectSpace, IgetListVideoEquipTargetInfoSpace } from '@/apis/modules/video-equip-import/model';
...
const rules = ref({
  accessSourceObjectId: [{ required: true, message: '选择监测指标' }],
  targetInfoId: [{ required: true, message: '选择接入对象' }]
});
...
const accessSourceOptions = ref<IgetListAccessSourceObjectSpace.Data[]>([]);
const targetInfoOptions = ref<IgetListVideoEquipTargetInfoSpace.Data[]>([]);

const isFirstStepFinished = computed(() => {
  return !!(formData.value.accessSourceObjectId && formData.value.targetInfoId);
});
...
const getAccessSource = async () => {
  const res = await apiVideoEquipmentObj.getListAccessSourceObject();
  accessSourceOptions.value = res.data;
};

const getTargetInfo = async () => {
  const res = await apiVideoEquipmentObj.getListVideoEquipTargetInfo();
  targetInfoOptions.value = res.data;
};
