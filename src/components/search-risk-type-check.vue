<template>
  <hl-selector class="w-full" ref="selector1" filterable clearable block @input="handleTreeInput" @clear="reset" @resize="resize" :placeholder="`请选择${$t('warningType')}`">
    <hl-scrollbar max-height="420px">
      <hl-tree ref="hlTreeSingle" show-checkbox :style="treeStyle" node-key="systemCode" :data="eventTypeData" :filter-node-method="filterNode" @check-change="chooseTree" class="p-md" :props="defaultProps" />
    </hl-scrollbar>
  </hl-selector>
</template>
...
const chooseTree = () => {
  ...
  // 选中列表 去除父节点 只选择leafNode
  const checkList = checkNodes
    .filter((itm: ItreeNode) => {
      return itm.leafNode;
    })
    .map((cur: ItreeNode) => {
