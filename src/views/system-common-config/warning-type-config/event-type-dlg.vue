<template>
  ...
  >
    ...
    >
      <hl-form-item label="预警类型名称" prop="eventTypeName">
        <hl-input
          v-model="formData.eventTypeName"
          placeholder="请输入预警类型名称"
          maxlength="20"
          show-word-limit
          block />
      </hl-form-item>
      <hl-form-item label="上级类型" v-if="hasParent">
        <!-- <span class="font-bold">{{ parentDisplayLabel ?? formatLabel(formData.parentFullSystemCode, formData.parentEventTypeName) }}</span> -->
        <hl-input
          :value="parentDisplayLabel ?? formatLabel(formData.parentFullSystemCode, formData.parentEventTypeName)"
          disabled
          block />
      </hl-form-item>
      ...
      <hl-form-item v-if="formData.leafNode" label="预警等级" prop="warningLevelTypeList">
        <hl-checkbox-group v-model="formData.warningLevelTypeList" dir="horizontal" gap="var(--xl)">
          <hl-checkbox
...
const rules = {
  eventTypeName: [
    { required: true, message: '预警类型名称不能为空' },
    {
      min: 1,
      max: 20,
      message: '长度小于20个字符',
  ...
  systemCode: [
    { required: true, message: '类型编号必须为2位数字' },
