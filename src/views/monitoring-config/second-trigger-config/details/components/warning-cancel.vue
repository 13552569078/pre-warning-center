<template>
  <hl-panel radius="0px" class="content w-full" borderless padding="var(--lg)" header-class="bg-content" body-padding="var(--lg)" body-class="display-flex items-left items-top flex-row content" footer-style>
    ...
    <template #default>
      <hl-row layout="grid" class="w-full w-60" count="count-2" gap="var(--lg)">
        ...
        <hl-col span="col col-span-2">
          <template v-if="detail?.clearType === 2">
            <span class="m-l-xxs font-bold">{{ getCancenType2Name(detail?.clearTypeValue) }}</span>
          </template>
          <template v-if="detail?.clearType === 3">
            <span>预警后，间隔一定时间再消警:</span>
            <span class="m-l-xxs font-bold">{{ getTriggerTimeName(detail?.clearTypeInterval) }}</span>
          </template>
          <template v-if="detail?.clearType === 4">
...
import { getTriggerTimeName, getTriggerNextName } from '@/views/monitoring-config/data-trigger-config/utils';
import { getClearTypeName, getCancenType2Name } from '@/views/monitoring-config/second-trigger-config/utils';
