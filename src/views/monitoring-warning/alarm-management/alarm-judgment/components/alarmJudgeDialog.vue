<template>
  <hl-dialog show-close v-model="dialogVisible" title="报警研判" width="1307px" header-class="border-transparent" body-class="display-flex items-left items-top flex-col gap-lg bg-light not-footer-dialog" body-padding="24px" destroy-on-close @close="emit('refreshList')">
    <hl-group full class="w-full" gap="var(--md)">
      <hl-group align class="static" style="width: 380px" dir="vertical" gap="var(--lg)" full>
        <hl-panel class="w-full border solid border-transparent static" padding="var(--xl)" header-class="border-transparent" body-class="display-flex items-left items-middle flex-row gap-md p-b-xl p-x-xl" body-style="padding-top: 0px;">
          <template #header>
          ...
          <template #default>
            <hl-form :model="judgeFormData" :disabled="judgeStatus === 1" ref="ruleFormRef" :rules="rules" class="w-full" gap="var(--md)" style="--form-label-color: var(--text-regular)">
              <hl-group align="items-left" class="w-full" gap="var(--md)" full style="--form-label-color: var(--text-regular)">
                <hl-form-item class="w-full" label="研判结果" prop="judgeStatus">
                  <hl-group align="items-left" class="w-full" dir="vertical" full>
                    <hl-radio-group v-model="judgeFormData.judgeStatus" style="padding-top: 5px; padding-bottom: 5px" align="items-left items-middle" class="w-full" gap="var(--xl)">
                ...
                <hl-form-item v-if="judgeFormData.judgeStatus === REAL_ALARM" class="w-full" label="新报警等级" prop="alarmLevelCode">
                  <hl-group align="items-top" class="w-full" full>
                    <uni-selector ref="newSearchAlarmLevelRef" v-model="judgeFormData.alarmLevelCode" dictType="ALARM_LEVEL_CODE" />
                  </hl-group>
                </hl-form-item>
              </hl-group>
              <hl-group v-if="judgeFormData.judgeStatus === REAL_ALARM" align="items-left" class="w-full" gap="var(--md)" full style="--form-label-color: var(--text-regular)">
                <hl-form-item class="w-full" label="处置规则">
                  <hl-group align="items-left" class="w-full" dir="vertical" full>
                    <hl-radio-group v-model="judgeFormData.handleOption" style="padding-top: 5px; padding-bottom: 5px" align="items-left items-middle" class="w-full" gap="var(--xl)">
              ...
              <hl-form-item class="w-full" label="描述">
                <hl-group align="items-left" class="w-full" dir="vertical" full>
                  <hl-input v-model="judgeFormData.judgeDesc" native-type="textarea" block placeholder="请输入描述" maxlength="200" show-word-limit></hl-input>
                </hl-group>
              </hl-form-item>
              <hl-form-item>
        ...
        <hl-panel class="w-full" padding="var(--xl)" header-class="border-transparent" body-class="display-flex items-left items-middle flex-row gap-md p-b-xl p-x-xl" body-style="padding-top: 0px;">
          <template #header>
            <div class="panel-header-left">
              <hl-icon class="static" type="primary" size="md">
                <FillOrnamentTitle4></FillOrnamentTitle4>
            ...
            <div class="panel-title">
              <h5 class="text-title w-full">报警详情</h5>
            </div>
          </template>
        ...
        <!--<hl-panel class="w-full" padding="var(&#45;&#45;xl)" header-class="border-transparent" body-class="display-flex items-left items-middle flex-row gap-md p-b-xl p-x-xl" body-style="padding-top: 0px;">
          ...
          &lt;!&ndash;    设备列表      &ndash;&gt;
          <div>
            <hl-tag :type="item.equipType?'primary':'success'" effect="light" v-for="item in equipList" :key="item.id" :class="['m-r-sm m-b-sm', item.equipType?'cursor-pointer':'']" @click="openVideo(item)">{{item.equipName}}</hl-tag>
          </div>
      ...
      <hl-group align="items-top" class="w-full" dir="vertical" gap="var(--lg)" full-x>
        <!--<hl-group class="p-lg bg-content w-full" align="items-left" dir="vertical" gap="var(&#45;&#45;sm)" full-x>
          <hl-form class="w-full" inline gap="var(&#45;&#45;lg)" item-gap="var(&#45;&#45;sm)" style="&#45;&#45;form-label-color: var(&#45;&#45;text-regular)">
            <hl-form-item label="风险分类">
              <hl-group align="items-left items-top">
                <event-type-selector v-model="searchFormData.eventTypeCode" body-class="w-md" ref="searchEventTypeRef" :use-for="1" />
              </hl-group>
            </hl-form-item>
            <hl-form-item label="报警等级">
              <hl-group align="items-left items-top">
                <uni-selector ref="searchAlarmLevelRef" body-class="w-sm" v-model="searchFormData.alarmLevel" dictType="ALARM_LEVEL_CODE" style="width: 202px" />
              </hl-group>
            </hl-form-item>
            <hl-form-item>
              <hl-group align="items-left items-middle" gap="var(&#45;&#45;md)">
        ...
        <hl-panel class="w-full content" borderless style="--panel-header-height: 74px" padding="var(--lg)" header-class="bg-content" body-class>
          <template #header>
            <div class="panel-header-left">
              <hl-icon class="static" type="primary" size="md">
                <FillOrnamentTitle4></FillOrnamentTitle4>
            ...
            <div class="panel-title">
              <h4 class="text-title w-full">历史报警趋势</h4>
            </div>
            <div class="panel-header-right">
              <hl-group v-for="item in dateList" :key="item.id" :class="['p-x-sm', item.bg, 'radius-md', 'cursor-pointer']" @click="changeDateList(item)" style="padding-top: 5px; padding-bottom: 5px" align="items-left items-middle" gap="var(--xs)"
                ><span :class="[item.color, 'text-center']">{{ item.name }}</span></hl-group
              >
            </div>
          </template>
          <template #default>
        ...
        <!--<hl-panel class="w-full content" borderless style="&#45;&#45;panel-header-height: 74px" padding="var(&#45;&#45;lg)" header-class="bg-content" body-class="p-x-lg" body-style="padding-top: 0px; padding-bottom: 0px;" footer-class="gap-md">
          <template #header>
            <div class="panel-header-left">
              <hl-icon class="static" type="primary" size="md">
                <FillOrnamentTitle4></FillOrnamentTitle4>
            ...
            <div class="panel-title">
              <hl-group full>
                <h4 class="text-title w-full">报警历史数据</h4>
                <hl-date-picker @change="getAlarmLogList" v-model="historySearchTime" format="YYYY-MM-DD HH:mm:ss" value-format="YYYY-MM-DD HH:mm:ss" start-placeholder="开始日期" end-placeholder="结束日期" range-separator="至" type="daterange" placeholder="选择启用时间" block />
              </hl-group>
            </div>
          </template>
          ...
          <template #footer>
            <hl-pagination :total="pagination.total" block align="items-left items-middle" @current-change="handleCurrentChange" layout="prev,pager,next"></hl-pagination>
          </template>
        </hl-panel>-->
      </hl-group>
    </hl-group>
    <!--<hl-dialog v-model="dialogVisible2" title="查看" width="600px" append-to-body>
      <video
        ...
        class="item-thumb w-full" controls></video>
    </hl-dialog>-->
  </hl-dialog>
</template>
...
import { IgetFindHistoryEventTrendSpace } from '@/apis/modules/ygf-alarm-info/model';

const emit = defineEmits(['refreshList']);
const REAL_ALARM = 1; // 真实报警
const FAKE_ALARM = 2; // 误报
const NEED_HANDLE = 2; // 需要处置
const NO_NEED_TO_HANDLE = 1; // 无需处置
...
watch(
  () => judgeFormData.value.judgeStatus,
  (status: number | null) => {
    // 误报，则处置规则设为「无需处置」
    if (status === FAKE_ALARM) {
      judgeFormData.value.handleOption = NO_NEED_TO_HANDLE;
    } else if (status === REAL_ALARM) {
      judgeFormData.value.handleOption = NEED_HANDLE;
    }
  }
);
...
const submitForm = () => {
  ruleFormRef.value.validate((valid: any) => {
    if (valid) {
      judgeFormData.value.eventInfoId = eventInfoId;
      apiYgfAlarmInfoObj.postJudgeEventInfo(judgeFormData.value).then(res => {
        HlMessageBox.confirm('报警信息已研判保存成功！', '提示', {
...
const rules = ref({
  judgeStatus: [{ required: true, message: '请选择研判结果', trigger: 'change' }],
  alarmLevelCode: [{ required: true, message: '请选择新报警等级', trigger: 'change' }]
});
...
const getAlarmInfo = async(id: string) => {
  const result = await apiYgfAlarmInfoObj.getEventInfo(id);
...
let trendList: IgetFindHistoryEventTrendSpace.Data = {
  timeList: [],
...
const getHistoryAlarmTrend = async() => {
  ...
  // };
  const { data } = await apiYgfAlarmInfoObj.getFindHistoryEventTrend(monitorUnitId || '1571823302426447873');
  trendList = data;
  renderChart();
};
...
const cols = ref([
  ...
  { title: '报警标题', prop: 'alarmTitle', showTooltip: true, tooltipProps: { minWidth: '240px' } },
  { title: '报警等级', prop: 'alarmLevelName', showTooltip: true, tooltipProps: { minWidth: '240px' } },
  { title: '报警开始时间', prop: 'startTime', minWidth: '10rem' },
