<template>
  <hl-dialog :model-value="true" class="md-dialog" body-class="result-total-body not-footer-dialog" width="604px" title="当前研判统计"
    @close="$emit('cancel')">
    <hl-panel borderless body-style="padding: var(--xxl);" class="m-b-lg">
      <hl-row gap="var(--xxl)">
        <hl-col span="col-12" style="height: calc(var(--lg) * 10)" class="display-flex items-middle items-center">
          <hl-progress show-as="circle" :percentage="Number(totalData.judgeRate.replace('%', ''))" color="#4CC09E"
            stroke-width="10px">
        ...
        <hl-col span="col-12" style="height: calc(var(--lg) * 10)">
          <hl-group align="items-center items-top" class="static" dir="vertical" gap="var(--md)" style="width: 100%;"
            full="full-x">
            <hl-group class="p-y-sm p-x-lg bg-light w-full" align="items-between items-middle"><span
                class="font-bold static">已研判</span>
              <hl-group align="items-left items-middle" gap="var(--xxs)">
                <h3 class="text-primary static">{{ totalData.judgeAlarmNumber }}</h3><span
                  class="text-secondary static">条</span>
              </hl-group>
            </hl-group>
            <hl-group class="p-y-sm p-x-lg bg-light w-full" align="items-between items-middle"><span
                class="font-bold static">待研判</span>
              <hl-group align="items-left items-middle" gap="var(--xxs)">
                <h3 class="text-primary static">{{ totalData.unJudgeAlarmNumber }}</h3><span
                  class="text-secondary static">条</span>
              </hl-group>
            </hl-group>
            <hl-group class="p-y-sm p-x-lg bg-success-light-5 w-full" align="items-between items-middle"><span
                class="font-bold static">当前研判率</span>
              <hl-group align="items-left items-middle" gap="var(--xxs)">
    ...
    <hl-panel class="w-full" header-style="gap: 0px;" borderless padding="var(--lg)" header-class="p-lg"
      body-class="display-flex items-left items-middle flex-row gap-xl p-b-lg"
      body-style="padding-top: 0px; padding-right: 32px; padding-left: 32px;"><template #header>
        ...
        <div
          :style="`padding: 30px; height: calc(var(--lg) * 6); width: 50%; background: url(${resultBg}) no-repeat; background-size: cover;`"
          class="display-flex">
          <div>
...
const getData2 = async (days: number) => {
  const result = await apiYgfAlarmInfoObj.getFindRecentJudgeResult(getDaysBetween(days).startTime, getDaysBetween(days).endTime);
  console.log(result);
  totalData2.value = result.data;
};
const getData = async () => {
  const { data } = await apiYgfAlarmInfoObj.getFindJudgeStatistic();
