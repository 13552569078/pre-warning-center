    ...
    >
      ...
      <template #toolbar-extra-after>
        <!-- <new-button
          ...
          @click-add="addFunc()"
        /> -->
        <fx-action-button label="新增房屋设施" type="main" @click-add="addFunc()"/>
      </template>
    </fx-page-table-v2>
...
import { IpostPageListSpace } from '@/apis/modules/special-facility/facility-building/model';
...
const cols = ref([
  ...
  { title: '房屋名称', prop: 'facilityName', minWidth: '200px', showTooltip: true, tooltipProps: { maxWidth: '260px' } },
  { title: '房屋类别', prop: 'buildingTypeName', minWidth: '200px', showTooltip: true, tooltipProps: { maxWidth: '260px' } },
  ...
  { title: '房屋状态', prop: 'facilityStateVal' },
...
const getSearchItems = async() => {
  // 下拉-设施类型
  const result1 = await apiDictObj.getListDictCustomVOsDict('FWSSLX_DT');
  const item1 = {
    type: 'select',
    label: '房屋类别',
    placeholder: '请选择房屋类别',
