...
import { IgetFacilityHeatSpace } from '@/apis/modules/special-facility/facility-heat/model';
...
const saveForm = async(data: IgetFacilityHeatSpace.Data) => {
  const searchParams = { ...data };
  props.action === 'add'
    ? await apiFacilityHeatObj.add(searchParams)
    : await apiFacilityHeatObj.postUpdate(searchParams);
  formCancel();
  backList();
};
...
onMounted(async() => {
  ...
  if (props.id.length) {
    const res = await apiFacilityHeatObj.getFacilityHeat(props.id);
