    ...
    >
      ...
      <template #toolbar-extra-after>
        <!-- <new-button
          ...
          @click-add="addFunc()"
        /> -->
        <fx-action-button label="新增桥梁设施" type="main" @click-add="addFunc()"/>
      </template>
    </fx-page-table-v2>
...
const cols = ref([
  ...
  { title: '桥梁名称', prop: 'facilityName', minWidth: '200px', showTooltip: true, tooltipProps: { maxWidth: '260px' } },
  // { title: '桥梁类别', prop: 'bridgeTypeName' },
  ...
  { title: '桥梁状态', prop: 'facilityStateVal' },
...
const getData = async({
  ...
  const { data } = await apiFacilityBridgeObj.postPageList(searchParams);
...
const deleteFunc = async(row: IpostPageListSpace.List) => {
  ...
  await apiFacilityBridgeObj.deleteF(row.id);
...
const selectionActions = reactive([
  {
    ...
    onClick: async() => {
      ...
      // 检验删除
      await checkDel(ids);
      await apiFacilityBridgeObj.delete(ids);
      HlMessage.success('删除成功');
      tableRef.value?.refresh();
    },
  },
