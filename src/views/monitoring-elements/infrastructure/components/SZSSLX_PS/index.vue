    ...
    >
      ...
      <template #toolbar-extra-after>
        <!-- <new-button
          ...
          @click-add="addFunc()"
        /> -->
        <fx-action-button label="新增排水设施" type="main" @click-add="addFunc()"/>
      </template>
    </fx-page-table-v2>
...
import { IpostPageListSpace } from '@/apis/modules/special-facility/facility-drainage/model';
...
const cols = ref([
  ...
  { title: '设施类别', prop: 'drainageTypeName', minWidth: '200px', showTooltip: true, tooltipProps: { maxWidth: '260px' } },
...
const getSearchItems = async() => {
  // 下拉-设施类型
  const result1 = await apiDictObj.getListDictCustomVOsDict('SZSSLX_PS');
  const item1 = {
    type: 'select',
    label: '设施类别',
    placeholder: '请选择设施类别',
    value: '',
    name: 'drainageTypeCode',
    prop: {
      label: 'dictName',
      value: 'dictCode',
    },
...
const getData = async({
  ...
  const { data } = await apiFacilityDrainageObj.postPageList(searchParams);
...
const deleteFunc = async(row: IpostPageListSpace.List) => {
  ...
  await apiFacilityDrainageObj.deleteF(row.id);
...
const selectionActions = reactive([
  {
    ...
    onClick: async() => {
      ...
      // 检验删除
      await checkDel(ids);
      await apiFacilityDrainageObj.delete(ids);
      HlMessage.success('删除成功');
      tableRef.value?.refresh();
    },
  },
