...
import { IgetFacilityWaterSpace } from '@/apis/modules/special-facility/facility-water/model';
...
const saveForm = async(data: IgetFacilityWaterSpace.Data) => {
  const searchParams = { ...data };
  props.action === 'add'
    ? await apiFacilityWaterObj.add(searchParams)
    : await apiFacilityWaterObj.postUpdate(searchParams);
  formCancel();
  backList();
};
...
onMounted(async() => {
  ...
  if (props.id.length) {
    const res = await apiFacilityWaterObj.getFacilityWater(props.id);
