    ...
    >
      ...
      <template #toolbar-extra-after>
        <!-- <new-button
          ...
          @click-add="addFunc()"
        /> -->
        <fx-action-button label="新增供水设施" type="main" @click-add="addFunc()"/>
      </template>
    </fx-page-table-v2>
    <detail-dlg
      v-if="detailDlgVisible"
      :id="currentId"
...
import { apiFacilityWaterObj } from '@/apis/modules/special-facility/facility-water';
import { IpostPageListSpace } from '@/apis/modules/special-facility/facility-water/model';
...
const cols = ref([
  ...
  { title: '设施类别', prop: 'waterTypeName', minWidth: '200px', showTooltip: true, tooltipProps: { maxWidth: '260px' } },
...
const getSearchItems = async() => {
  // 下拉-设施类型
  const result1 = await apiDictObj.getListDictCustomVOsDict('SZSSLX_GS');
  const item1 = {
    type: 'select',
    label: '设施类别',
    placeholder: '请选择设施类别',
...
const getData = async({
  ...
  const { data } = await apiFacilityWaterObj.postPageList(searchParams);
...
const deleteFunc = async(row: IpostPageListSpace.List) => {
  ...
  await apiFacilityWaterObj.deleteF(row.id);
...
const selectionActions = reactive([
  {
    ...
    onClick: async() => {
      ...
      // 检验删除
      await checkDel(ids);
      await apiFacilityWaterObj.delete(ids);
      HlMessage.success('删除成功');
      tableRef.value?.refresh();
    },
  },
