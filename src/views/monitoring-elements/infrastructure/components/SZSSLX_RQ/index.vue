...
import { IpostPageListSpace } from '@/apis/modules/special-facility/facility-gas/model';
...
const cols = ref([
  ...
  { title: '设施类别', prop: 'gasTypeName', minWidth: '200px', showTooltip: true, tooltipProps: { maxWidth: '260px' } },
...
const getSearchItems = async() => {
  // 下拉-设施类型
  const result1 = await apiDictObj.getListDictCustomVOsDict('SZSSLX_RQ');
  const item1 = {
    type: 'select',
    label: '设施类别',
    placeholder: '请选择设施类别',
...
const getData = async({
  ...
  const { data } = await apiFacilityGasObj.postPageList(searchParams);
...
const deleteFunc = async(row: IpostPageListSpace.List) => {
  ...
  await apiFacilityGasObj.deleteF(row.id);
...
const selectionActions = reactive([
  {
    ...
    onClick: async() => {
      ...
      // 检验删除
      await checkDel(ids);
      await apiFacilityGasObj.delete(ids);
      HlMessage.success('删除成功');
      tableRef.value?.refresh();
    },
  },
