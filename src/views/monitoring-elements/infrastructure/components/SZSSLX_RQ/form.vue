<template>
  <hl-panel style="--panel-body-padding:calc(var(--xxl) + var(--md));">
    ...
    <hl-form ref="formRef" :disabled="dialogsType === 'detail'" :model="form" :rules="formRules" :label-position="['items-left', 'items-left']" :width="['col-24', 'col-24']">
      <hl-row class="show-col" gap="var(--md)">
        <hl-col span="col-16">
          <hl-form-item label="设施名称:" prop="facilityName">
            <hl-input v-model.trim="form.facilityName" placeholder="设施名称" maxlength="100" show-word-limit block />
          </hl-form-item>
        </hl-col>
        <hl-col span="col-8">
          <hl-form-item label="设施类别:" prop="gasTypeCode">
            <hl-select v-model="form.gasTypeCode" placeholder="设施类别" block filterable clearable>
              <hl-option :label="item.dictName" :value="item.dictCode" v-for="(item, index) in typeList" :key="index" />
            </hl-select>
          </hl-form-item>
        </hl-col>
      </hl-row>
        ...
        <hl-col span="col-8">
          <hl-form-item label="养护等级:" prop="maintainLevelCode">
            <hl-select v-model="form.maintainLevelCode" placeholder="养护等级" block filterable clearable>
              <hl-option :label="item.dictName" :value="item.dictCode" v-for="(item, index) in levelList" :key="index" />
            </hl-select>
          </hl-form-item>
        </hl-col>
        <hl-col span="col-8">
      ...
      <hl-row class="show-col" gap="var(--md)">
        <hl-col span="col-8">
          <hl-form-item label="地址:" prop="address">
            <hl-input v-model.trim="form.address" placeholder="地址" maxlength="200" show-word-limit block class="address-input">
              <template #suffix>
      ...
      <hl-row class="show-col" gap="var(--md)">
        ...
        <hl-col span="col-8">
          <hl-form-item label="设计单位:" prop="designUnitName">
      ...
      <hl-row class="show-col" gap="var(--md)">
        <hl-col span="col-8">
          <hl-form-item label="建成时间:" prop="completedTime">
            <hl-date-picker class="w-full" value-format="YYYY-MM-DD HH:mm:ss" v-model="form.completedTime" type="date" placeholder="建成时间" />
          </hl-form-item>
        </hl-col>
        <hl-col span="col-8">
          <hl-form-item label="总造价（万元）:" prop="totalCost">
            <hl-input v-model.trim="form.totalCost" placeholder="总造价（万元）" block />
          </hl-form-item>
        </hl-col>
        <hl-col span="col-8">
          <hl-form-item label="设施状态:" prop="facilityStateCode">
            <hl-select v-model="form.facilityStateCode" placeholder="设施状态" block filterable clearable>
              <hl-option :label="item.dictName" :value="item.dictCode" v-for="(item, index) in statusList" :key="index" />
            </hl-select>
          </hl-form-item>
        </hl-col>
      </hl-row>
      <hl-row class="show-col" gap="var(--md)">
        <hl-col span="col-24">
          <hl-form-item label="备注:" prop="gasDesc">
            <hl-input
            native-type="textarea"
            :rows="4"
            v-model.trim="form.gasDesc"
            placeholder="备注"
            block
            maxlength="500"
            show-word-limit
...
import { IgetFacilityGasSpace } from '@/apis/modules/special-facility/facility-gas/model';
...
const formRules = {
  ...
  gasTypeCode: [ruleRequired],
  address: [ruleRequired],
  longitude: [ruleRequired, ruleNumber],
  latitude: [ruleRequired, ruleNumber],
  areaCode: [ruleRequired],
  directeUnitCode: [ruleRequired],
  maintainUnitCode: [ruleRequired],
  maintainLevelCode: [ruleRequired],
  completedTime: [ruleRequired],
  facilityStateCode: [ruleRequired],
  totalCoverArea: [ruleRequired],
...
const saveForm = async(data: IgetFacilityGasSpace.Data) => {
  const searchParams = { ...data };
  props.action === 'add'
    ? await apiFacilityGasObj.add(searchParams)
    : await apiFacilityGasObj.postUpdate(searchParams);
  formCancel();
  backList();
};
...
onMounted(async() => {
  ...
  if (props.id.length) {
    const res = await apiFacilityGasObj.getFacilityGas(props.id);
