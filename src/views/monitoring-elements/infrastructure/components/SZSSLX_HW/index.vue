    ...
    >
      ...
      <template #toolbar-extra-after>
        <!-- <new-button
          ...
          @click-add="addFunc()"
        /> -->
        <fx-action-button label="新增环卫设施" type="main" @click-add="addFunc()"/>
      </template>
    </fx-page-table-v2>
    <detail-dlg
...
import { apiFacilitySanitationObj as api } from '@/apis/modules/special-facility/facility-sanitation';
import { IpostPageListSpace } from '@/apis/modules/special-facility/facility-sanitation/model';
...
const cols = ref([
  ...
  { title: '设施类别', prop: 'sanitationTypeName', minWidth: '200px', showTooltip: true, tooltipProps: { maxWidth: '260px' } },
...
const getSearchItems = async() => {
  // 下拉-设施类型
  const result1 = await apiDictObj.getListDictCustomVOsDict('SZSSLX_HW');
  const item1 = {
    type: 'select',
    label: '设施类别',
    placeholder: '请选择设施类别',
    value: '',
    name: 'sanitationTypeCode',
    prop: {
      label: 'dictName',
      value: 'dictCode',
    },
...
const getData = async({
  ...
  const { data } = await api.postPageList(searchParams);
