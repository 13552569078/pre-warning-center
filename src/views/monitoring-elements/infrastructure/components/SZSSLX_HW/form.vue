        ...
        <hl-col span="col-24">
          ...
          >
            <hl-input
              ...
              :rows="4"
              v-model.trim="form.sanitationDesc"
              placeholder="备注"
              block
              maxlength="500"
              show-word-limit
...
import { IaddSpace, IpostUpdateSpace } from '@/apis/modules/special-facility/facility-sanitation/model';
...
onMounted(async() => {
  if (props.id.length) {
    const res = await api.getFacilitySanitation(props.id);
