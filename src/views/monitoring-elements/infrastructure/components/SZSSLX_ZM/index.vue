    ...
    >
      ...
      <template #toolbar-extra-after>
        <!-- <new-button
          ...
          @click-add="addFunc()"
        /> -->
        <fx-action-button label="新增照明设施" type="main" @click-add="addFunc()"/>
      </template>
    </fx-page-table-v2>
...
import { IpostPageListSpace } from '@/apis/modules/special-facility/facility-light/model';
...
const cols = ref([
  ...
  { title: '路灯名称', prop: 'facilityName', minWidth: '200px', showTooltip: true, tooltipProps: { maxWidth: '260px' } },
  // { title: '路灯类别', prop: 'bridgeTypeName', minWidth: '200px', showTooltip: true, tooltipProps: { maxWidth: '260px' }  },
  ...
  { title: '路灯状态', prop: 'facilityStateVal' },
...
const getData = async({
  ...
  const { data } = await apiFacilityLightObj.postPageList(searchParams);
...
const deleteFunc = async(row: IpostPageListSpace.List) => {
  ...
  await apiFacilityLightObj.deleteF(row.id);
...
const selectionActions = reactive([
  {
    ...
    onClick: async() => {
      ...
      // 检验删除
      await checkDel(ids);
      await apiFacilityLightObj.delete(ids);
      HlMessage.success('删除成功');
      tableRef.value?.refresh();
    },
  },
