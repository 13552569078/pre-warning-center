    ...
    >
      ...
      <template #toolbar-extra-after>
        <!-- <new-button
          ...
          @click-add="addFunc()"
        /> -->
        <fx-action-button label="新增管廊设施" type="main" @click-add="addFunc()"/>
      </template>
    </fx-page-table-v2>
...
import { IpostPageListSpace } from '@/apis/modules/special-facility/facility-pipeline/model';
...
const cols = ref([
  ...
  { title: '设施类别', prop: 'pipelineTypeName', minWidth: '200px', showTooltip: true, tooltipProps: { maxWidth: '260px' } },
...
const getSearchItems = async() => {
  // 下拉-设施类型
  const result1 = await apiDictObj.getListDictCustomVOsDict('SZSSLX_GL');
  const item1 = {
    type: 'select',
    label: '设施类别',
    placeholder: '请选择设施类别',
    value: '',
    name: 'pipelineTypeCode',
    prop: {
      label: 'dictName',
      value: 'dictCode',
    },
