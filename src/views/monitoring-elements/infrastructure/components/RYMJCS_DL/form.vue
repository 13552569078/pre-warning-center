    ...
    >
      ...
      >
        ...
        <hl-col span="col-8">
          <hl-form-item
            label="场所类别:"
            prop="crowdedTypeCode"
          >
            <hl-select
              v-model="form.crowdedTypeCode"
              placeholder="场所类别"
              block
              filterable
              clearable
...
import { IgetFacilityCrowdedSpace } from '@/apis/modules/special-facility/facility-crowded/model';
...
const saveForm = async(data: IgetFacilityCrowdedSpace.Data) => {
  const searchParams = { ...data };
  props.action === 'add'
    ? await apiFacilityCrowdedObj.add(searchParams)
    : await apiFacilityCrowdedObj.postUpdate(searchParams);
  formCancel();
  backList();
};
...
onMounted(async() => {
  ...
  if (props.id.length) {
    const res = await apiFacilityCrowdedObj.getFacilityCrowded(props.id);
