...
import { IpostPageListSpace } from '@/apis/modules/special-facility/facility-crowded/model';
...
const cols = ref([
  ...
  { title: '设施类别', prop: 'crowdedTypeName', minWidth: '200px', showTooltip: true, tooltipProps: { maxWidth: '260px' } },
...
const getSearchItems = async() => {
  // 下拉-设施类型
  const result1 = await apiDictObj.getListDictCustomVOsDict('RYMJCS_DL');
  const item1 = {
    type: 'select',
    label: '场所类型',
    placeholder: '请选择场所类型',
    value: '',
    name: 'crowdedTypeCode',
    prop: {
      label: 'dictName',
      value: 'dictCode',
    },
  ...
  // 下拉-养护等级
  const result3 = await apiDictObj.getListDictCustomVOsDict('MAINTAIN_LEVEL');
  const item3 = {
    type: 'select',
    label: '养护等级',
    placeholder: '请选择养护等级',
    ...
    options: [...result3.data],
  };
  searchItems.push(item3);
...
const getData = async({
  ...
  const { data } = await apiFacilityCrowdedObj.postPageList(searchParams);
...
const deleteFunc = async(row: IpostPageListSpace.List) => {
  ...
  await apiFacilityCrowdedObj.deleteF(row.id);
...
const selectionActions = reactive([
  {
    ...
    onClick: async() => {
      ...
      // 检验删除
      await checkDel(ids);
      await apiFacilityCrowdedObj.delete(ids);
      HlMessage.success('删除成功');
      tableRef.value?.refresh();
    },
  },
