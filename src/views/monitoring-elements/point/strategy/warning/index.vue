                ...
                <hl-form v-show="isOpen" class="factor-form p-b-lg rule-item" :ref="el => addPinDuanFormRef(el)" :rules="getPinDuanRules(frequency)" :model="frequency" :label-position="['items-left', 'items-middle']" :validate-on-rule-change="false">
                  <hl-form-item prop="alarmWarn">
                    <hl-group dir="vertical" class="w-full">
                      <hl-form-item label="预警策略" class="m-t-md">
                        <hl-select v-model="frequency.warnStrategyId" placeholder="请选择报警策略" block filterable clearable @change="valadateForm">
                          <hl-option :key="item.monitorStrategyId" :label="item.strategyName" :value="item.monitorStrategyId" v-for="item in filterOptions(warnList, eventitem.eventTypeCode)" />
                        </hl-select>
                      </hl-form-item>
                    </hl-group>
                  </hl-form-item>
...
const checkAlarmWarn = (formData: { alarmStrategyId: string; warnStrategyId: string }) => (_: any, __: any, callback: (err?: Error) => void) => {
  if (!formData.warnStrategyId) {
    callback(new Error('请选择预警策略'));
  } else {
    callback();
  }
};
...
const getWarns = async(eventTypeCodes?: string) => {
  const { data } = await apiMonitorStrategyObj.getFindMonitorStrategyPage(CommonUtils.toQuery({ pageSize: 9999, currentPage: 1, useFor: 2, eventTypeCodes: [eventTypeCodes] }));
  warnList.value = data.list;
};
